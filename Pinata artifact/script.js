...
defaultImage.crossOrigin = 'anonymous'; // Allow CORS if needed
...
const pinata = {
    x: ropeAnchor.x,
    y: canvas.height / 2 - 40, // moved upward so image is further away from "HIT ME"
...
function applySwirlToImage(sourceImage, damagePoints) {
    if (!sourceImage || !damagePoints || damagePoints.length === 0) return sourceImage;
    ...
    damagePoints.forEach(damage => {
        ...
        if (w > 0 && h > 0) {
            const imageData = dCtx.getImageData(x, y, w, h);
...
function drawPinata(pivotX, pivotY) {
    ...
    if (imageToDraw) {
        try {
            ...
            // slightly scale up the image so it appears larger than before
            const scaleMul = 1.08;
            const drawW = bodyW * scaleMul;
            const drawH = bodyH * scaleMul;
...
canvas.addEventListener('click', (e) => {
    ...
    if (dist < pinata.width / 2 + 60 && !isBroken) {
        ...
        const scaleMul = 1.08;
        const drawW = pinata.width * scaleMul;
        const drawH = pinata.height * scaleMul;
...
window.addEventListener('resize', () => {
    canvas.width = window.innerWidth;
    canvas.height = window.innerHeight;
    ropeAnchor.x = canvas.width / 2;
    ropeAnchor.y = -200; // keep rope starting offscreen
    pinata.x = ropeAnchor.x;
    pinata.y = canvas.height / 2 - 40;
    string.startY = ropeAnchor.y;
    string.length = pinata.y - ropeAnchor.y;
});
